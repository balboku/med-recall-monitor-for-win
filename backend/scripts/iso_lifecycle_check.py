"""
ISO 標準 Life cycle 版本檢查（虛擬瀏覽器）— 手動命令列工具
============================================================
...
實際的瀏覽器與解析邏輯集中於 crawlers/iso_browser.py（後端 API、爬蟲共用），本檔僅為
可手動執行的命令列包裝。www.iso.org 受 Cloudflare 防護，需以 nodriver + 本機 Chrome 通過挑戰。

使用方式（於 backend 目錄下）：
    python scripts/iso_lifecycle_check.py "ISO 10993-1"
    python scripts/iso_lifecycle_check.py "ISO 10993-1" --current 2018
    set ISO_BROWSER_HEADLESS=1 && python scripts/iso_lifecycle_check.py "ISO 13485"  # 嘗試無頭
...
def main():
    ap = argparse.ArgumentParser(description="ISO 標準 Life cycle 版本檢查（虛擬瀏覽器）")
